            ...
            try
            {
                while (true)
                {
                   ...
                   // mensagem = Encoding.ASCII.GetString(bytes);                 //Converte o byte para string.
                    usuario = (Usuario) BinarioSerializacao.Deserializar(bytes);

                    if (usuario == null) {
                        byte[] paraEnviar = Encoding.ASCII.GetBytes("usuario ou senha incorretos");
                    ...
                    if (usuario.Login == "admin" && usuario.Senha == "admin")
                    {
                        ...
                        byte[] paraEnviar = Encoding.ASCII.GetBytes("sucesso ao conectar");
                        sCliente.Send(paraEnviar);
                        while (true)
                        {
                           
                            bytes = new byte[1024];                                 //Número máximo que poderá receber 1 MegaByte
                            sCliente.Receive(bytes);                                //Passa valores recebidos para o array de bytes
                            mensagem = deBytesParaString(bytes);                    //Recebe as mensagem
